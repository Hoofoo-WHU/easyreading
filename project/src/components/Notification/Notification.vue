<template>
  <div class="notification fixed" v-if="show" :style="setStyle" transition="slide">
    <div class="delete" v-if="!options.autoClose" @click="close()"></div>
    <div class="content">{{ options.content }}</div>
  </div>
  <div class="countdown" v-else-if="show && options.autoClose && options.countdownBar" :style="setTime" :class="barControl"></div>
</template>
